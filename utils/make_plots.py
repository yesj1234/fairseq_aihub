...
if __name__ == "__main__":
    ...
    gender_plot_y = ["여성", "남성"]
    gender_plot_x = [len(df[df["speaker_gender"] == "여성"]), len(df[df["speaker_gender"] == "남성"])]
    ...
    word_phrase_plot_data = dict(sorted(word_phrase_plot_data.items(), key = lambda x: x[1], reverse=True))
