...
class MUSTC(Dataset):
    ...
    LANGUAGES = ["de", "es", "fr", "it", "nl", "pt", "ro", "ru", "en"]

    def __init__(self, root: str, lang: str, split: str) -> None:
        assert split in self.SPLITS and lang in self.LANGUAGES
        _root = Path(root) / f"kr-{lang}" / "data" / split
...
def process_joint(args):
    ...
    assert all((cur_root / f"kr-{lang}").is_dir() for lang in MUSTC.LANGUAGES), \
    ...
    with NamedTemporaryFile(mode="w") as f:
        for lang in MUSTC.LANGUAGES:
            tsv_path = cur_root / f"kr-{lang}" / f"train_{args.task}.tsv"
    ...
    for lang in MUSTC.LANGUAGES:
        for split in MUSTC.SPLITS:
            src_path = cur_root / f"kr-{lang}" / f"{split}_{args.task}.tsv"
